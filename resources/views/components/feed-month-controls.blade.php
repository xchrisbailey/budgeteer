...
<div class="flex items-center justify-between w-full px-3 py-2 mb-2 md:justify-center">
    <div>
        <a href="{{ route('dashboard', ['month' => $previousDate['month'], 'year' => $previousDate['year']]) }}" class="inline-flex p-1 mr-2 transition duration-200 ease-in-out bg-gray-100 rounded-full
    ...
    <div>
        <a href="{{ route('dashboard', ['month' => $nextDate['month'], 'year' => $nextDate['year']]) }}"
            class="inline-flex p-1 ml-2 transition duration-200 ease-in-out bg-gray-100 rounded-full shadow hover:bg-gray-200 ">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
